...
from utils.helpers import pad_up_to
...
def TransitionDown(inputs, n_filters, dropout_p=0.0, scope=None):
    """
...
def TransitionUp(block_to_upsample, skip_connection, n_filters_keep, scope=None):
    ...
    with tf.name_scope(scope) as sc:
        # Upsample
        up_block = slim.conv2d_transpose(block_to_upsample, n_filters_keep, kernel_size=[3, 3], stride=[2, 2],
                                  activation_fn=None)
        # Concatenate with skip connection
        up_block = pad_up_to(up_block, tf.shape(skip_connection), 0)
        up_block = tf.concat([up_block, skip_connection], axis=-1)

        return up_block


def build_fc_densenet(inputs, num_classes, preset_model='FC-DenseNet56', n_filters_first_conv=48,
                      n_pool=5, growth_rate_factor=0.5, n_layers_per_block=4, dropout_p=0.0, scope=None,):
    """
    ...
    Returns:
      Fc-DenseNet model
    """
    if preset_model == 'FC-DenseNet34':
        n_pool = 5
        growth_rate = 8
        n_layers_per_block = 2
    elif preset_model == 'FC-DenseNet45':
        n_pool = 5
        growth_rate = 8
        n_layers_per_block = 3
    elif preset_model == 'FC-DenseNet58':
        n_pool = 5
        growth_rate = 8
        n_layers_per_block = [1, 2, 4, 7, 9, 12, 9, 7, 4, 2, 1]
    elif preset_model == 'FC-DenseNet56':
        n_pool = 5
        # growth_rate = 8  # default
        growth_rate = 4
        n_layers_per_block = 4
    elif preset_model == 'FC-DenseNet67':
        n_pool = 5
        # growth_rate = 16 # default
        growth_rate = 3
        ...
        raise ValueError("Unsupported FC-DenseNet model '%s'. "
                         "This function only supports FC-DenseNet56, FC-DenseNet67, and FC-DenseNet103" % preset_model)
    ...
    for i in range(n_pool):
        # Dense Block
        stack, _ = DenseBlock(stack, n_layers_per_block[i], growth_rate, dropout_p, scope='denseblock_%d' % (i+1))
        n_filters += growth_rate * n_layers_per_block[i]
        # At the end of the dense block, the current stack is stored in the skip_connections list
        skip_connection_list.append(stack)

        # Transition Down
        stack = TransitionDown(stack, n_filters, dropout_p, scope='transitiondown_%d' %(i+1))
