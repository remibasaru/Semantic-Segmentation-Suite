...
def AtrousSpatialPyramidPoolingModule(inputs, depth=256):
    ...
    with tf.variable_scope('Atrous_conv'):
        with tf.variable_scope('Atrous_conv_1'):
            atrous_pool_block_1 = slim.conv2d(inputs, depth, [1, 1], activation_fn=None)

        with tf.variable_scope('Atrous_conv_2'):
            atrous_pool_block_6 = slim.conv2d(inputs, depth, [3, 3], rate=6, activation_fn=None)

        with tf.variable_scope('Atrous_conv_3'):
            atrous_pool_block_12 = slim.conv2d(inputs, depth, [3, 3], rate=12, activation_fn=None)

        with tf.variable_scope('Atrous_conv_4'):
            atrous_pool_block_18 = slim.conv2d(inputs, depth, [3, 3], rate=18, activation_fn=None)

        with tf.variable_scope('concat_atrous'):
            net = tf.concat((atrous_pool_block_1, atrous_pool_block_6, atrous_pool_block_12, atrous_pool_block_18), axis=3)

    with tf.variable_scope('concat'):
        net = tf.concat((image_features, net), axis=3)

    return net


def build_deeplabv3_plus(inputs, num_classes, preset_model='DeepLabV3+', frontend="ResNet101", load_pre_train=False,
    ...
    logits, end_points, frontend_scope, init_fn, stable_params = \
        frontend_builder.build_frontend(inputs, frontend, pre_trained_dir=pre_trained_dir, is_training=is_training,
                                        load_pre_train=load_pre_train, requested_stages=[2, 4])

    label_size = tf.shape(inputs)[1:3]

    encoder_features = end_points['stage2']
